...
GEO_FILE = "meshes/FussbodenheizungSegmentNass.geo"
...
THERMAL_CONDUCTIVITIES = {
    ...
    "polystyrene": dlfn.Constant(0.032),                # Styropor: https://www.energieheld.de/daemmung/daemmstoffe/styropor-eps#eigenschaften
    "pur_alukaschiert": dlfn.Constant(0.023),           # PUR/PIR Platten Alukaschiert: https://www.baudiscount-paderborn.de/PUR-/-PIR-Daemmung-WLG-023-alukaschiert
...
NCC_BOUNDARIES = {
    "Coverfloor": NCC_BOUNDARY(external_temperature=22.0, heat_transfer_coefficient=5.),
    "Hot Inlet": NCC_BOUNDARY(external_temperature=45.0, heat_transfer_coefficient=3000.0),
    "Cold Outlet": NCC_BOUNDARY(external_temperature=40.0, heat_transfer_coefficient=3000.0),
}
...
class Mesh:
    def __init__(self, geo_file=GEO_FILE):
        ...
        self.mesh, self.facet_markers, self.facet_marker_map, self.cell_markers, self.cell_markers_map = self.load_mesh(geo_file)

        # Compute the largest width and height of the domain
        self.w, self.h = self.get_domain_sizes()
    ...
    def load_mesh(geo_file):
        ...
        :param geo_file:
        :return: 5-tuple containing the mesh and the assiciated MeshFunctions for the facet_markers and cell_markers
        """
...
class Solver:
    def __init__(self, mesh: Mesh, function_space: dlfn.FunctionSpace, dirichlet_boundaries: Dict):
        ...
        self.dirichlet_bcs = [get_dirichlet(mesh, function_space, facet_name, value) for facet_name, value in dirichlet_boundaries.items()]
        # Initiate a variable for the linear form
    ...
    def save_grad(self, filename="temperature_gradient.pvd"):
        ...
        :return:
        """
        solution_gradient = dlfn.grad(self.w)
        solution_gradient = dlfn.project(solution_gradient)
    ...
    def save_heat_conduction(self, filename="heat_conduction.pvd"):
        ...
        :return:
        """
        raise NotImplementedError("Pleeease implement me!")
...
def main(args):
    ...
    boundary_conditions.PeriodicBoundary.lh = mesh.w
